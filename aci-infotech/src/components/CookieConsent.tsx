...
/**
 * CookieConsent — GDPR/CCPA banner. The animations are CSS-only
 * (slide-up keyframe + max-height transition), no framer-motion. The
 * previous implementation imported `motion` and `AnimatePresence`,
 * which pulled the entire framer-motion bundle (~40 KB gz) into the
 * initial JS chunk on every page on the site even though this
 * component sits behind `dynamic({ ssr: false })`. Removing the
 * dependency drops that weight site-wide.
 */
...
export default function CookieConsent() {
  ...
  return (
    <div
      className="aci-cookie-banner fixed bottom-0 left-0 right-0 z-[9999] p-4 md:p-6"
      role="dialog"
      aria-modal="false"
      aria-labelledby="aci-cookie-banner-title"
    >
          ...
          <div className="p-4 md:p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 id="aci-cookie-banner-title" className="text-lg font-semibold text-gray-900 mb-2">
                ...
                {/* Cookie Details (Expandable). max-height transition is
                    a long-standing CSS-only pattern that approximates an
                    auto-height open. Slight overshoot is fine because the
                    panel is short. */}
                <div
                  className="aci-cookie-details overflow-hidden"
      ...
      <style>{`
        .aci-cookie-banner {
          animation: aci-cookie-slide-up 320ms cubic-bezier(0.16, 1, 0.3, 1) both;
        }
        @keyframes aci-cookie-slide-up {
        ...
        .aci-cookie-details[data-state="open"] {
          max-height: 480px;
          opacity: 1;
        }
        @media (prefers-reduced-motion: reduce) {
          .aci-cookie-banner { animation: none; }
          .aci-cookie-details { transition: none; }
        }
      `}</style>
    </div>
  );
