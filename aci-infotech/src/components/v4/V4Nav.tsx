...
export default function V4Nav() {
  ...
  // At the top the nav sits over the dark video hero (light treatment);
  // once scrolled it becomes a light glass bar (dark treatment).
  const dark = !scrolled;
  const linkColor = dark ? 'rgba(255,255,255,0.86)' : 'var(--v4-ink)';
    ...
    >
      ...
      >
        <Link href="/" aria-label="ACI Infotech home" style={{ display: 'flex', alignItems: 'center' }}>
          <Image
            src={dark ? '/aci-infotech-logo-white.png' : '/aci-infotech-logo.png'}
            alt="ACI Infotech"
            width={119}
            height={38}
            priority
