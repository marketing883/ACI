...
const VAULT = [
  ...
  { d: 23, title: 'Post-Acquisition ERP Unification', cat: 'Integration', slug: 'post-acquisition-consolidation' },
...
function Label({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/55 ${className}`}
    >
      <Sparkle className="h-3 w-3 text-[#A3E635]" strokeWidth={1.5} aria-hidden="true" />
      {children}
    </span>
  );
}
...
function LogoTile({ id }: { id: string }) {
  return (
    <span className="pb-glass flex h-14 w-14 shrink-0 items-center justify-center rounded-xl md:h-16 md:w-16">
      <Image
        src={LOGO_PATH[id]}
        alt={id}
        width={120}
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-16 sm:px-6 md:px-10 md:py-20 lg:px-14">
        ...
        <div className="mb-10 flex flex-col gap-6 md:mb-12 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2
              className={`text-4xl font-normal tracking-tight text-white sm:text-5xl ${headingClass}`}
              style={{ lineHeight: 1.05 }}
            >
              Patterns we&apos;ve run&nbsp;before.
            </h2>
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/60 md:text-[15px]">
          ...
          <Link
            href="/playbooks"
            className="pb-glass pb-glass-blur group inline-flex shrink-0 items-center gap-2 self-start rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/[0.06] sm:px-6 sm:py-3 md:self-auto"
          >
        ...
        {/* Bento grid */}
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5 lg:grid-cols-3 lg:grid-rows-2 lg:min-h-[600px]">
          {/* A — Vault index (tall) */}
          <div
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-8 p-6 md:p-7">
              <div className="flex items-center justify-between">
                <Label>Playbook vault</Label>
                <span className="text-[11px] font-medium text-white/40">{PATTERN_COUNT} patterns</span>
              </div>
                  ...
                  {VAULT.map((p) => (
                    <li key={p.slug}>
                      <Link
                        ...
                        className="group grid grid-cols-[3rem_1fr_auto] items-center gap-3 border-t border-white/10 py-3 transition-colors hover:bg-white/[0.03]"
                      >
                        <span className="font-mono text-sm font-semibold text-[#A3E635]">{p.d}×</span>
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-medium text-white/90 transition-colors group-hover:text-white">
                            {p.title}
                          </span>
                          <span className="text-[11px] uppercase tracking-wide text-white/40">{p.cat}</span>
                        </span>
                        <ArrowUpRight
                          size={16}
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-6 p-6 md:p-7">
              ...
              <div>
                ...
                <p className="mt-3 text-sm text-white/80">Enterprise deployments, all documented.</p>
                <p className="text-xs text-white/45">across {PATTERN_COUNT} architecture patterns</p>
              </div>
            </div>
          </div>
          ...
          >
            <div className="pb-glass absolute inset-0 rounded-2xl" aria-hidden="true" />
            <div className="relative flex h-full min-h-[220px] flex-col justify-between gap-6 p-6 md:p-7">
              <div className="flex items-center justify-between">
                <Label>Runs on</Label>
                <span className="text-[11px] font-medium text-white/40">the stack we know cold</span>
              </div>
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-5 p-6 md:p-7">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full border border-[#A3E635]/30 bg-[#84CC16]/10 px-2.5 py-0.5 font-mono text-xs font-semibold text-[#A3E635]">
                  {FLAGSHIP.deployments}× deployed
                </span>
              </div>
              <div>
                <p className={`text-xl font-medium leading-snug text-white ${headingClass}`}>
                  {FLAGSHIP.title}
                </p>
                <p className="mt-2 max-w-md text-[13px] leading-relaxed text-white/60">{FLAGSHIP.blurb}</p>
              </div>
              <div className="flex items-end justify-between gap-4">
                <div className="flex gap-5">
                  {FLAGSHIP.outcomes.map((o) => (
                    <div key={o.label}>
                      <div className={`text-2xl font-semibold text-white ${headingClass}`}>{o.metric}</div>
                      <div className="text-[11px] uppercase tracking-wide text-white/45">{o.label}</div>
                    </div>
                  ))}
                </div>
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-6 p-6 md:p-7">
              ...
              <span className="text-sm font-semibold text-[#A3E635]">Talk to an architect</span>
            </div>
          </Link>
        </div>
      </div>
