...
function Label({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-white/60 ${className}`}
    >
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5 lg:grid-cols-3 lg:grid-rows-2 lg:min-h-[600px]">
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-8 p-6 md:p-7">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-medium text-white/45">{PATTERN_COUNT} patterns</span>
              </div>

              <div>
                <p className="mb-5 max-w-sm text-base leading-relaxed text-white/70">
                  ...
                  {VAULT.map((p) => (
                    <li key={p.slug}>
                      <Link
                        ...
                        className="group grid grid-cols-[3.25rem_1fr_auto] items-center gap-3 border-t border-white/10 py-3.5 transition-colors hover:bg-white/[0.03]"
                      >
                        <span className="font-mono text-base font-semibold text-[#A3E635]">{p.d}×</span>
                        <span className="min-w-0">
                          <span className="block truncate text-base font-medium text-white/90 transition-colors group-hover:text-white">
                            {p.title}
                          </span>
                          <span className="text-xs uppercase tracking-wide text-white/45">{p.cat}</span>
                        </span>
                        <ArrowUpRight
                          size={18}
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-6 p-6 md:p-7">
              ...
              <div>
                ...
                <p className="mt-3 text-base text-white/85 md:text-lg">Enterprise deployments, all documented.</p>
                <p className="text-sm text-white/50">across {PATTERN_COUNT} architecture patterns</p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative flex h-full min-h-[220px] flex-col justify-between gap-6 p-6 md:p-7">
              <div className="flex items-center justify-between">
                <Label>Runs on</Label>
                <span className="text-xs font-medium text-white/45">the stack we know cold</span>
              </div>
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-5 p-6 md:p-7">
              ...
              <div>
                ...
                <p className="mt-2.5 max-w-md text-[15px] leading-relaxed text-white/65">{FLAGSHIP.blurb}</p>
              </div>
              <div className="flex items-end justify-between gap-4">
                <div className="flex gap-6">
                  {FLAGSHIP.outcomes.map((o) => (
                    <div key={o.label}>
                      <div className={`text-3xl font-bold text-white ${headingClass}`}>{o.metric}</div>
                      <div className="text-xs uppercase tracking-wide text-white/50">{o.label}</div>
                    </div>
                  ))}
                </div>
          ...
          >
            ...
            <div className="relative flex h-full flex-col justify-between gap-6 p-6 md:p-7">
              ...
              <div>
                ...
                <p className="mt-2.5 max-w-xs text-base leading-relaxed text-white/80">
