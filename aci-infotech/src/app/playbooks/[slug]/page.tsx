...
const playbooksData: Record<string, PlaybookDetail> = {
  ...
  'agentic-ai-deployment': {
    ...
    overview: 'Everyone has an AI agent demo. Few have agents in production. The gap between a working prototype and a reliable enterprise deployment is vast—filled with hallucinations, security risks, unpredictable costs, and users who don\'t trust the output. This playbook, refined through 18 enterprise deployments, provides the architecture for moving AI agents from impressive demos to mission-critical workflows. We cover single-agent deployment, multi-agent orchestration, guardrails that actually work, human-in-the-loop design that users accept, and cost management at scale. The result: AI agents that your operations team trusts and your CFO approves.',
    challengePattern: [
      'Agent hallucination causing incorrect outputs at production scale—fine in demos, catastrophic in production',
      'Orchestrating multi-agent systems without cascading failures when one agent produces bad output',
      'Designing effective human-in-the-loop checkpoints that add safety without destroying efficiency',
      'Managing latency requirements for real-time workflows where users expect instant responses',
      'Preventing data leakage and prompt injection attacks in systems handling sensitive information',
      'Controlling costs as usage scales—what costs $50/day in pilot can become $50,000/day in production',
    ],
    solutionApproach: [
      'Use-Case Assessment: Identify workflows where agents add value vs. where traditional automation suffices. Not every process needs AI.',
      'Agent Architecture: Design agent structure—single vs. multi-agent, tool access, memory management, context windows, model selection.',
      'Guardrails Framework: Output validation, hallucination detection, confidence scoring, fallback behaviors. Agents that know when they don\'t know.',
      'Human-in-the-Loop: Strategic checkpoints for high-stakes decisions. Async review queues. Feedback loops that improve agent performance.',
      'Production Deployment: Monitoring, logging, A/B testing, gradual rollout. Treat agents like any critical production system.',
      'Cost & Performance: Token optimization, caching strategies, model routing, usage monitoring. Keep CFO happy as you scale.',
    ],
    keyLearnings: [
      'Start with single-agent before attempting multi-agent orchestration. Multi-agent systems multiply complexity exponentially.',
      'Guardrails matter more than model selection. A well-guarded GPT-4 beats an unguarded Claude every time.',
      'Evaluation frameworks are non-negotiable for production. If you can\'t measure agent performance, you can\'t improve it.',
      'Human-in-the-loop is a feature, not a crutch. Users trust agents more when they know a human reviews edge cases.',
      'Track cost per transaction from day one. Usage patterns in production never match pilot assumptions.',
      'Agent memory management is harder than expected. Context windows fill fast. Retrieval strategies matter enormously.',
    ],
    outcomes: [
      { metric: '60%', description: 'Manual effort reduced', detail: 'AI agents handle routine workflow steps, freeing humans for judgment calls' },
      { metric: '94%', description: 'Task accuracy', detail: 'With proper guardrails, agent accuracy exceeds human performance on routine tasks' },
      { metric: '3.2x', description: 'First-year ROI', detail: 'Efficiency gains offset infrastructure and API costs within 12 months' },
      { metric: 'Zero', description: 'Critical failures', detail: 'Guardrails and human-in-the-loop prevent catastrophic errors in production' },
    ],
    industries: ['Financial Services', 'Retail', 'Technology', 'Healthcare'],
    technologies: ['LangChain', 'LlamaIndex', 'Azure OpenAI', 'AWS Bedrock', 'Kubernetes', 'Apache Kafka', 'Vector DBs', 'Python'],
    category: 'AI & ML',
    timeline: '6-12 months typical',
    teamSize: '10-20 consultants',
    relatedPlaybooks: ['enterprise-ai-governance', 'self-service-analytics'],
    downloadAvailable: false,
  },
  'enterprise-ai-governance': {
    ...
    overview: 'Your company deployed 47 ML models last year. How many are still in production? Who approved them? What data did they train on? Can you prove they\'re not discriminating? If regulators ask tomorrow, how long until you have answers? Most enterprises have no idea. Models proliferate faster than governance can track. Shadow AI runs in spreadsheets and notebooks. Production models drift without monitoring. This playbook, proven across 14 regulated enterprise deployments, provides the framework for AI governance that actually works—automated, not bureaucratic. We cover model inventory, bias detection, explainability for different audiences, audit trails, and multi-jurisdiction compliance. The result: AI you can defend to regulators, boards, and customers.',
    challengePattern: [
      'No unified AI governance framework across the organization—each team does their own thing or nothing at all',
      'Model bias manifesting differently across diverse datasets—what\'s fair for one demographic may discriminate against another',
      'Navigating regulatory requirements across multiple jurisdictions—EU AI Act, CCPA, HIPAA, SEC, all with different requirements',
      'Stakeholders demanding explainability at different levels—executives want summaries, auditors want details, users want confidence',
      'No audit trail for model decisions and predictions—when something goes wrong, you can\'t reconstruct what happened',
      'Shadow AI proliferating faster than governance can catch—ChatGPT in spreadsheets, unauthorized models in production',
    ],
    solutionApproach: [
      'AI Asset Inventory: Comprehensive catalog of all models—production, development, shadow. You can\'t govern what you can\'t see.',
      'Risk Classification: Tier models by risk level. High-stakes decisions get more scrutiny. Low-risk models move fast.',
      'Bias Detection Pipeline: Automated fairness testing across protected classes. Catch bias before production, monitor for drift after.',
      'Explainability Layer: SHAP values for data scientists. Natural language summaries for executives. Confidence scores for users.',
      'Audit Trail Infrastructure: Every prediction logged with inputs, outputs, model version, and lineage. Reconstruct any decision.',
      'Compliance Automation: Map requirements to controls. Generate compliance reports automatically. Stay ahead of changing regulations.',
    ],
    keyLearnings: [
      'Governance must be automated, not manual review gates. Manual processes create bottlenecks that teams will route around.',
      'Explainability requirements vary dramatically by stakeholder. Build multiple views: technical, executive, customer-facing.',
      'Bias testing must run on production data, not just training sets. Distribution shift in production reveals bias training data hid.',
      'Regulations are a moving target—build for adaptability. EU AI Act today, something else tomorrow. Modular controls adjust.',
      'Shadow AI is the biggest risk most organizations ignore. Unsanctioned models cause incidents. Shine light on shadow AI.',
      'Model risk tiering prevents governance from becoming a bottleneck. Not every model needs board approval.',
    ],
    outcomes: [
      { metric: '100%', description: 'Audit coverage', detail: 'Every model decision logged and traceable for regulatory inquiries' },
      { metric: '85%', description: 'Bias incidents reduced', detail: 'Automated detection catches fairness issues before production deployment' },
      { metric: '6+', description: 'Frameworks supported', detail: 'Single governance platform handles HIPAA, GDPR, CCPA, EU AI Act, and more' },
      { metric: '40%', description: 'Faster approvals', detail: 'Automated compliance checks accelerate model deployment without sacrificing safety' },
    ],
    industries: ['Financial Services', 'Healthcare', 'Insurance', 'Government'],
    technologies: ['MLflow', 'Weights & Biases', 'SHAP', 'Fairlearn', 'Great Expectations', 'Apache Airflow', 'Evidently AI', 'Python'],
    category: 'AI & ML',
